...
export default function ClientChatPage() {
  ...
  return (
    <div className="flex flex-col" style={{ height: "100dvh" }}>
      {/* Chat Header */}
      <header className="flex items-center gap-3 px-4 py-3 bg-white border-b border-border sticky top-0 z-50">
        <button
        ...
        <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 bg-gray-200 flex items-center justify-center text-gray-500 font-medium">
          {other?.profileImage ? (
        ...
        <div className="flex-1 min-w-0">
          <h2 className="font-semibold text-textPrimary truncate">{other?.name || "Loading..."}</h2>
          {conversation?.bookingId && (
            <p className="text-xs text-gray-500">
              {conversation.bookingId.serviceName} •{" "}
              {new Date(conversation.bookingId.date).toLocaleDateString("en-US", { month: "short", day: "numeric" })} •{" "}
              {conversation.bookingId.timeSlot}
            </p>
          )}
        </div>
      </header>
                    ...
                    <div
                      className={`px-4 py-2.5 rounded-2xl ${
                        isMine
                          ...
                          : "bg-white text-textPrimary border border-border rounded-bl-md"
                      }`}
                    >
      ...
      <form onSubmit={handleSend} className="p-4 bg-white border-t border-border">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-all active:scale-95"
          >
