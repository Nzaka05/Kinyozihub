...
export default function RescheduleResponsePage() {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (res.data?.accepted) {
        setAccepted(true);
        setTimeout(() => {
          router.push(`/client/bookings/${params.id}/reschedule-confirmed`);
        }, 1000);
      }
    } catch (err: any) {
  ...
  // The proposer is the other party
  const proposer = booking.proposedBy === 'barber' ? booking.barber : booking.client;
  const proposerName = proposer?.name || 'Your barber';
            ...
            <div className="flex justify-between items-start">
              ...
              {proposer?.rating && (
                <div className="flex items-center gap-1 text-primary">
                  ...
                  <span className="font-semibold text-sm">{proposer.rating}</span>
                </div>
              )}
            </div>
        ...
        {booking.proposedMessage && (
          <section className="space-y-4">
            <h3 className="font-semibold text-sm text-gray-500 uppercase tracking-widest">
              {booking.proposedBy === 'barber' ? "Barber's Note" : "Client's Note"}
            </h3>
